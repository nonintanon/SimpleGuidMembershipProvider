...
namespace nonintanon.Security
{
    public class SimpleRoleProvider : RoleProvider
    {
        ...
        {
            List<Guid> userIds = new List<Guid>(usernames.Length);
            foreach (string username in usernames) {
                Guid id = SimpleMembershipProvider.GetUserId(db, SafeUserTableName, SafeUserNameColumn, SafeUserIdColumn, username);
                if (id == Guid.Empty) {
        ...
        {
            List<Guid> roleIds = new List<Guid>(roleNames.Length);
            foreach (string role in roleNames) {
                Guid id = FindRoleId(db, role);
                if (id == Guid.Empty) {
        ...
        private static Guid FindRoleId(IDatabase db, string roleName)
        {
            ...
            return Guid.Parse(result[0].ToString());
        }
